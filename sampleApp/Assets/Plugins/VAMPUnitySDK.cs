...
public class VAMPUnitySDK : MonoBehaviour
{
    ...
    [DllImport("__Internal")]
    private static extern void VAMPUnityGetCountryCode(GetCountryCodeCallback callback);

    [DllImport("__Internal")]
    private static extern void VAMPUnityIsEUAccess(IsEUAccessCallback callback);
    ...
    [DllImport("__Internal")]
    private static extern void VAMPUnitySetChildDirected(bool childDirected);

    [DllImport("__Internal")]
    private static extern bool VAMPUnityIsChildDirected();

    [DllImport("__Internal")]
    private static extern void VAMPUnitySetUnderAgeOfConsent(int underAgeOfConsent);
    ...
    public static void initialize(string placementID)
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            var player = new AndroidJavaClass(UnityPlayerClass);
    ...
    public static void getCountryCode(GetCountryCodeCallback callback)
    {
        ...
        if (Application.platform == RuntimePlatform.IPhonePlayer)
        {
            VAMPUnityGetCountryCode(VAMPDidGetCountryCode);
        }
#elif UNITY_ANDROID
        if (Application.platform == RuntimePlatform.Android)
        {
    ...
    public static void isEUAccess(IsEUAccessCallback callback)
    {
        ...
        if (Application.platform == RuntimePlatform.IPhonePlayer)
        {
            VAMPUnityIsEUAccess(VAMPIsEUAccess);
        }
#elif UNITY_ANDROID
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            vampCls.CallStatic("isEUAccess", activity, new UserConsentListener());
        }
#endif
    }
    ...
    public static void setUserConsent(ConsentStatus status)
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            var consentStatusCls = new AndroidJavaClass("jp.supership.vamp.VAMPPrivacySettings$ConsentStatus");
            var vampCls = new AndroidJavaClass(VampClass);
    ...
    /// (AdMob、FAN、Tapjoy)
    /// </summary>
    /// <param name="childDirected">同意年齢未満かどうか</param>
    public static void setChildDirected(bool childDirected)
    {
#if UNITY_IOS
        if (Application.platform == RuntimePlatform.IPhonePlayer)
        {
            VAMPUnitySetChildDirected(childDirected);
        }
#elif UNITY_ANDROID
        if (Application.platform == RuntimePlatform.Android) 
        {
            var vampCls = new AndroidJavaClass(VampClass);
            vampCls.CallStatic("setChildDirected", childDirected);
        }
#endif
    }

    /// <summary>
    /// <see cref="setChildDirected(bool)"/>で設定した値を取得します。
    /// </summary>
    /// <returns><see cref="setChildDirected(bool)"/>で設定した値</returns>
    public static bool isChildDirected()
    {
#if UNITY_IOS
        if (Application.platform == RuntimePlatform.IPhonePlayer)
        {
            return VAMPUnityIsChildDirected();
        }
#elif UNITY_ANDROID
        if (Application.platform == RuntimePlatform.Android)
        {
            var vampCls = new AndroidJavaClass(VampClass);
            return vampCls.CallStatic<bool>("isChildDirected");
        }
#endif
        return false;
    }
    ...
    /// </summary>
    /// <param name="underAgeOfConsent">同意年齢未満かどうか</param>
    public static void setUnderAgeOfConsent(UnderAgeOfConsent underAgeOfConsent)
    {
#if UNITY_IOS
        if (Application.platform == RuntimePlatform.IPhonePlayer)
        {
            VAMPUnitySetUnderAgeOfConsent((int)underAgeOfConsent);
        }
#elif UNITY_ANDROID
        if (Application.platform == RuntimePlatform.Android)
        {
            var underAgeOfConsentCls = new AndroidJavaClass("jp.supership.vamp.VAMPPrivacySettings$UnderAgeOfConsent");
            var vampCls = new AndroidJavaClass(VampClass);
            vampCls.CallStatic("setUnderAgeOfConsent", underAgeOfConsentCls.GetStatic<AndroidJavaObject>(underAgeOfConsent.ToString()));
        }
#endif
    }
    /// <summary>
    ...
    public static void setTargeting(Targeting targeting)
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            var user = new AndroidJavaObject("jp.supership.vamp.VAMPTargeting");
            var genderCls = new AndroidJavaClass("jp.supership.vamp.VAMPTargeting$Gender");
...
#elif UNITY_ANDROID && !UNITY_EDITOR
            var configCls = new AndroidJavaClass(VAMPConfigurationClass);
